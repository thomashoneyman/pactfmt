...
#[cfg(test)]
mod tests {
    use pactfmt_macros::test_corpus;
    ...
    fn lex_correctly(input: &str) -> bool {
        lex(input).into_iter().all(|token| token.is_ok())
    }

    fn check_corpus_file(input: &str) {
        assert!(lex_correctly(input));
    }

    test_corpus!(check_corpus_file);
